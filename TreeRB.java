...
public class TreeRB<T extends Comparable<T>> {
    ...
    public void insert(T item, Comparator<T> c) {
        ...
        while (currentNode != null) {
            ...
            if (c.compare(currentNode.value, item) > 0) {
        ...
        if (c.compare(previousNode.value, item) > 0)
    ...
    public T find(T value, Comparator<T> c) {
        Node currentNode = root;
        while (currentNode != null) {
            if (c.compare(currentNode.value, value) == 0)
    ...
    private void RPrintTree(Node current, int level) {
        if (current == null)
            ...
            if (current.type)
                System.out.println(level + " : " + "\u001B[31m" + current.value + "\u001B[0m");
            else
                System.out.println(level + " : " + current.value);
    ...
    public void inOrderPrint() {
        ...
        while (!queue.isEmpty()) {
            ...
            if (current == root || current.father != null) {
                if (current.type)
                    System.out.print("\u001B[31m" + current.toString() + "\u001B[0m ");
                else
